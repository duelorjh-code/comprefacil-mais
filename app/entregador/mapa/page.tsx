...
export default function EntregadorMapa() {
  ...
  return (
    <div style={s.wrap}>
      ...
      {pedido && (
        <div style={s.info}>
          <div style={s.infoRow}><span style={s.infoL}>📦 Retirar em</span>
            <span style={s.infoV}>
          ...
          <div style={s.infoRow}><span style={s.infoL}>🏠 Entregar em</span><span style={s.infoV}>{pedido.endereco_entrega}</span></div>
          <div style={s.infoRow}><span style={s.infoL}>👤 Cliente</span><span style={s.infoV}>{pedido.clientes?.perfis?.nome ?? '—'}</span></div>
          <a href={`https://wa.me/55${pedido.clientes?.perfis?.telefone?.replace(/\D/g,'')}?text=Olá, sou o entregador do seu pedido CompreFácil+. Estou a caminho!`}
            target="_blank" rel="noreferrer" style={s.btnWhats}>💬 WhatsApp cliente</a>
        </div>
      )}
      {!pedido && (
        <div style={s.semPedido}><p style={{ fontSize: 13, color: '#64748B', textAlign: 'center' as const }}>Nenhum pedido em andamento.</p></div>
      )}
