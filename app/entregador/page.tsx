...
export default function EntregadorPage() {
  ...
  async function carregar(eid: string) {
    const { data } = await supabase.from('pedidos')
      .select(`id, status, total, taxa_entrega, distancia_km, sla_minutos, tempo_limite, endereco_entrega, codigo_confirmacao,
  ...
  async function aceitar(pedidoId: string) {
    ...
    navigator.geolocation.getCurrentPosition(async pos => {
      await supabase.from('entregadores').update({ lat_atual: pos.coords.latitude, lng_atual: pos.coords.longitude }).eq('id', entId)
      await supabase.from('pedidos').update({ entregador_id: entId, status: 'a_caminho' }).eq('id', pedidoId)
      carregar(entId)
    })
  }
  ...
  async function recusar(pedidoId: string, just?: string) {
    ...
    setModal(null); setJustificativa('')
    carregar(entId)
  }

  async function confirmarEntrega(pedidoId: string, codigoPedido: string) {
    if (codigo !== codigoPedido) return alert('Código incorreto. Peça ao cliente o código de 4 dígitos.')
    await supabase.from('pedidos').update({ status: 'entregue' }).eq('id', pedidoId)
    setModalCodigo(null); setCodigo('')
    carregar(entId)
  }
        ...
        <div style={s.lista}>
          {pedidos.map(p => (
            <div key={p.id} style={{ ...s.card, ...(p.status==='pronto' ? s.cardNovo : {}) }}>
              {p.status === 'pronto' && <div style={s.novoBadge} className="anim-blink">🔔 Nova entrega disponível</div>}
              ...
              <div style={s.info}>
                ...
                <div style={s.infoItem}><span style={s.infoL}>Retirar em</span><span style={s.infoV}>{p.parceiros?.nome_fantasia} — {p.parceiros?.endereco}, {p.parceiros?.numero}</span></div>
                <div style={s.infoItem}><span style={s.infoL}>Cliente</span><span style={s.infoV}>{p.clientes?.perfis?.nome}</span></div>
                <div style={s.infoItem}><span style={s.infoL}>Distância</span><span style={s.infoV}>{p.distancia_km}km · ~{p.sla_minutos}min</span></div>
                <div style={s.infoItem}><span style={s.infoL}>Sua taxa</span><span style={{ ...s.infoV, color:VERDE, fontWeight:800 }}>{formatBRL(p.taxa_entrega)}</span></div>
              </div>
              ...
              <div style={s.acoes}>
                ...
                {p.status === 'a_caminho' && <>
                  <button onClick={() => router.push('/entregador/mapa')} style={{ ...s.btn, background:'#EEF2FF', color:AZUL }}>🗺️ Ver mapa</button>
                  <a href={`tel:${p.clientes?.perfis?.telefone}`} style={{ ...s.btn, background:'#22C55E20', color:VERDE, textDecoration:'none', textAlign:'center' as const }}>📞 Cliente</a>
        ...
        <div style={s.overlay} onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div style={s.modalCard} className="anim-fadeUp">
            ...
            <div style={s.modalAcoes}>
              ...
              <button onClick={() => recusar(modal, justificativa || undefined)} style={{ ...s.btn, background:'#EF444420', color:VERMELHO }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalCodigo && (
        <div style={s.overlay} onClick={e => e.target === e.currentTarget && (setModalCodigo(null), setCodigo(''))}>
          <div style={s.modalCard} className="anim-fadeUp">
            ...
            <p style={{ fontSize:13, color:TEXTO_MEIO }}>Peça ao cliente o código de 4 dígitos que ele recebeu.</p>
            ...
            <div style={s.modalAcoes}>
              ...
              }} style={{ ...s.btn, background:VERDE, color:'#fff' }}>✓ Confirmar</button>
            </div>
          </div>
        </div>
      )}
