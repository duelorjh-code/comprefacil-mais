...
export default function EntregadorPage() {
  ...
  async function initEntregador() {
    ...
    const { data: e } = await supabase
      .from('entregadores').select('id, status, validado').eq('usuario_id', user.id).single()
    if (!e) return
    setEntId(e.id)
  ...
  function initMapa(L: any) {
    ...
    const map = L.map(mapRef.current, { zoomControl: false }).setView([LAT_DEFAULT, LNG_DEFAULT], 14)
    ...
    el.innerHTML = '<div style="background:#1B2F5E;width:20px;height:20px;border-radius:50%;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:12px">🏍️</div>'
    const icone = L.divIcon({ html: el.innerHTML, iconSize: [20, 20], className: '' })
    motoMarker.current = L.marker([LAT_DEFAULT, LNG_DEFAULT], { icon: icone }).addTo(map)
  }

  // Atualiza posição da moto no mapa
  useEffect(() => {
    if (!coords || !mapObj.current || !motoMarker.current) return
    motoMarker.current.setLatLng([coords.lat, coords.lng])
    if (online) mapObj.current.setView([coords.lat, coords.lng], 15)
  }, [coords, online])

  // Mapa cinza quando offline
  useEffect(() => {
    const canvas = mapRef.current?.querySelector('canvas') as HTMLCanvasElement | null
    if (canvas) canvas.style.filter = online ? 'none' : 'grayscale(80%)'
  }, [online])
    ...
    <div style={s.wrap}>
      ...
      <div style={s.mapaWrap}>
        ...
        {coords && (
          <button style={s.btnCentralizar}
            onClick={() => mapObj.current?.setView([coords.lat, coords.lng], 15)}>
            📍
          </button>
        )}
      </div>
      ...
      <div style={s.lista}>
        <div style={s.listaHeader}>
          ...
          {!validado && <span style={s.badgePendente}>⏳ Aguardando validação</span>}
        </div>
        ...
        {pedidos.map(p => (
          <div key={p.id} style={{ ...s.card, ...(p.status === 'pronto' ? s.cardNovo : {}) }}>
            ...
            <div style={s.cardTop}>
              ...
              <span style={{ ...s.pill, background: p.status === 'a_caminho' ? DOURADO + '20' : '#22C55E20', color: p.status === 'a_caminho' ? DOURADO : VERDE }}>
            ...
            <div style={s.info}>
              ...
              <div style={s.infoItem}>
                ...
                <span style={s.infoV}>{p.parceiros?.nome_fantasia ? `${p.parceiros.nome_fantasia} — ${p.parceiros.endereco ?? ''}, ${p.parceiros.numero ?? ''}`.trim().replace(/,\s*$/, '') : '—'}</span>
              </div>
              <div style={s.infoItem}><span style={s.infoL}>Cliente</span><span style={s.infoV}>{p.clientes?.perfis?.nome ?? '—'}</span></div>
              <div style={s.infoItem}><span style={s.infoL}>Distância</span><span style={s.infoV}>{p.distancia_km ? `${p.distancia_km}km · ~${Math.round(p.distancia_km * 4 + 10)}min` : '—'}</span></div>
              <div style={s.infoItem}><span style={s.infoL}>Sua taxa</span><span style={{ ...s.infoV, color: VERDE, fontWeight: 800 }}>{formatBRL(p.taxa_entrega ?? 6)}</span></div>
            </div>
            <div style={s.itens}>
            ...
            <div style={s.acoes}>
              {p.status === 'pronto' && <>
                <button onClick={() => aceitar(p.id)} style={{ ...s.btn, background: AZUL, color: '#fff' }}>✓ Aceitar entrega</button>
                <button onClick={() => setModal(p.id)} style={{ ...s.btn, background: '#EF444420', color: VERMELHO }}>✕ Recusar</button>
              </>}
              {p.status === 'a_caminho' && <>
                <a href={`tel:${p.clientes?.perfis?.telefone}`}
                  style={{ ...s.btn, background: '#22C55E20', color: VERDE, textDecoration: 'none', textAlign: 'center' as const }}>📞 Cliente</a>
                <button onClick={() => setModalCodigo(p.id)} style={{ ...s.btn, background: VERDE, color: '#fff' }}>✓ Confirmar entrega</button>
              </>}
            </div>
          </div>
        ))}
      ...
      {modal && (
        <div style={s.overlay} onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div style={s.modalCard}>
            ...
            <textarea style={s.textarea} placeholder="Motivo (opcional)…" value={justificativa} onChange={e => setJustificativa(e.target.value)} rows={3} />
            <div style={s.modalAcoes}>
              <button onClick={() => setModal(null)} style={s.btnCancelar}>Voltar</button>
              <button onClick={() => recusar(modal, justificativa || undefined)} style={{ ...s.btn, background: '#EF444420', color: VERMELHO }}>✕ Confirmar recusa</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalCodigo && (
        <div style={s.overlay} onClick={e => e.target === e.currentTarget && (setModalCodigo(null), setCodigo(''))}>
          <div style={s.modalCard}>
            ...
            <input style={{ ...s.textarea, textAlign: 'center', fontSize: 28, fontWeight: 800, letterSpacing: 8, height: 'auto', padding: '16px' }}
              type="text" inputMode="numeric" maxLength={4} value={codigo}
              onChange={e => setCodigo(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="0000" />
            <div style={s.modalAcoes}>
              <button onClick={() => { setModalCodigo(null); setCodigo('') }} style={s.btnCancelar}>Cancelar</button>
              <button onClick={() => confirmarEntrega(modalCodigo!)} style={{ ...s.btn, background: VERDE, color: '#fff' }}>✓ Confirmar</button>
            </div>
          </div>
        </div>
      )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  mapaWrap:        { position: 'relative', height: 260, flexShrink: 0 },
  mapa:            { width: '100%', height: '100%' },
  mapaOverlay:     { position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', zIndex: 10 },
  ...
  mapaBadgeAtivo:  { position: 'absolute', bottom: 10, left: 10, right: 10, background: AZUL, color: '#fff', borderRadius: 10, padding: '8px 12px', fontSize: 12, fontWeight: 700, textAlign: 'center' as const, pointerEvents: 'none', zIndex: 10 },
  btnCentralizar:  { position: 'absolute', top: 10, left: 10, background: '#fff', border: 'none', borderRadius: 8, width: 36, height: 36, fontSize: 18, cursor: 'pointer', boxShadow: '0 2px 6px rgba(0,0,0,0.2)', zIndex: 10 },
