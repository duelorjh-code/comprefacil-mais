...
import { AZUL, DOURADO, VERDE, VERMELHO, LARANJA, TEXTO, TEXTO_MEIO, formatBRL } from '@/lib/constants'
...
export default function AdminDashboard() {
  ...
  async function carregar() {
    ...
    const [
      ...
      supabase.from('pedidos').select('total').eq('status','entregue').gte('criado_em', hojeISO),
...
const s: Record<string, React.CSSProperties> = {
  ...
  cardValor: { fontSize: 26, fontWeight: 800, lineHeight: 1 },
  ...
  graficoTitulo: { fontSize: 14, fontWeight: 700, color: TEXTO, marginBottom: 16 },
  barras: { display: 'flex', alignItems: 'flex-end', gap: 6, height: 120 },
  barraItem: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 },
  barraWrap: { flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end', minHeight: 90 },
  barra: { width: '100%', borderRadius: '4px 4px 0 0', transition: 'height 0.3s ease', minHeight: 4 },
  barraLabel: { fontSize: 10, color: TEXTO_MEIO, fontWeight: 600 },
  barraValor: { fontSize: 10, color: AZUL, fontWeight: 800 },
  statusLista: { display: 'flex', flexDirection: 'column', gap: 10 },
  statusItem: { display: 'flex', alignItems: 'center', gap: 8 },
  statusDot: { width: 8, height: 8, borderRadius: '50%', flexShrink: 0 },
  statusLabel: { fontSize: 12, color: TEXTO_MEIO, width: 110, flexShrink: 0 },
  statusBarWrap: { flex: 1, height: 6, background: '#F1F5F9', borderRadius: 3, overflow: 'hidden' },
  statusBar: { height: '100%', borderRadius: 3, transition: 'width 0.4s ease' },
  statusQtd: { fontSize: 12, fontWeight: 700, color: TEXTO, width: 24, textAlign: 'right' as const },
}
